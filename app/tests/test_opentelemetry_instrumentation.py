...
class TestOpenTelemetryInstrumentation:
    ...
    def test_ollama_chat_with_opentelemetry_instrumentation(self, mock_chat_interface, mock_tracer, mock_span):
        ...
        with patch("opentelemetry.trace.get_tracer", return_value=mock_tracer), patch(
            ...
            mock_tracer.start_as_current_span.assert_called_once_with("gen_ai.chat.completions")
            ...
            mock_client.chat.assert_called_with(model=model, messages=messages, stream=False)
    ...
    def test_ollama_chat_without_opentelemetry_fallback(self, mock_chat_interface):
        ...
        # Setup mock Ollama response
        mock_response = {"message": {"content": "Fallback response", "role": "assistant"}}
    ...
    def test_ollama_chat_error_handling_with_span(self, mock_chat_interface, mock_tracer, mock_span):
    ...
    def test_span_attribute_setting_with_different_models(self, mock_chat_interface, mock_tracer, mock_span):
        ...
        with patch("opentelemetry.trace.get_tracer", return_value=mock_tracer), patch("opentelemetry.trace.Status"), patch(
    ...
    def test_response_metadata_extraction(self, mock_chat_interface, mock_tracer, mock_span):
        ...
        with patch("opentelemetry.trace.get_tracer", return_value=mock_tracer), patch("opentelemetry.trace.Status"), patch(
            ...
            for i, test_case in enumerate(test_responses):
                ...
                mock_span.set_attribute.assert_any_call("gen_ai.response.length", test_case["expected_length"])
    ...
    def test_threading_timeout_mechanism(self, mock_chat_interface):
        ...
        with patch("opentelemetry.trace.get_tracer", side_effect=ImportError("No OTel")):
            ...
            def run_chat():
                return mock_chat_interface._chat_with_ollama(messages, "tinyllama:latest")
            ...
            assert not chat_thread.is_alive(), "Chat thread should have completed due to timeout"
    ...
    def test_result_queue_handling(self, mock_chat_interface):
        ...
        expected_response = {"message": {"content": "Queue result test", "role": "assistant"}}
    ...
    def test_no_result_in_queue_error(self, mock_chat_interface):
        ...
        with patch("threading.Thread") as mock_thread_class, patch("queue.Queue") as mock_queue_class, patch(
            ...
            def queue_side_effect():
                return mock_result_queue if mock_queue_class.call_count == 1 else mock_exception_queue
            ...
            with pytest.raises(Exception, match="Ollama call completed but no result received"):
                mock_chat_interface._chat_with_ollama(messages, "tinyllama:latest")
    ...
    def test_genai_tracer_availability(self, mock_chat_interface, mock_tracer):
        ...
        with patch("opentelemetry.trace.get_tracer", return_value=mock_tracer) as mock_get_tracer:
    ...
    def test_manual_instrumentation_vs_auto_instrumentation(self, mock_chat_interface, mock_tracer, mock_span):
        """Test that manual instrumentation works alongside potential auto-instrumentation."""
        # Setup mock response
        mock_response = {"message": {"content": "Manual instrumentation test", "role": "assistant"}}
        mock_client = Mock()
        mock_client.chat.return_value = mock_response
        ollama_mock.Client.return_value = mock_client
