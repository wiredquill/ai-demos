...
# usage or cost charts are produced for the app. Re-create the counter here and
# bump it on every successful chat completion.
_requests_counter = None
# The SUSE AI metric bindings query cost under gen_ai.client.operation.cost
# (gen_ai_client_operation_cost_USD_* in StackState). openlit records cost as
# gen_ai.usage.cost, which no binding consumes, so also record the SUSE name.
