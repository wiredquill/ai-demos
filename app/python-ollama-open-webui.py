...
class ObservableAPIServer:
    ...
    def setup_routes(self):
        ...
        @self.app.route("/ask", methods=["GET", "POST", "OPTIONS"])
        def ask_endpoint():
            ...
            try:
                ...
                response = (
                    self.chat_interface.ollama_openai_client.ask_endpoint_handler(
                        test_prompt, "llama3.2:latest"
                    )
                )
...
class ChatInterface:
        ...
        try:
            ...
            if initialize_openlit():
                ...
                self.enhanced_observability = {
                    "token_tracking": os.getenv(
                        "TOKEN_TRACKING_ENABLED", "true"
                    ).lower()
                    == "true",
                    "cost_tracking": os.getenv("COST_TRACKING_ENABLED", "true").lower()
                    == "true",
                    "model_metrics": os.getenv("MODEL_METRICS_ENABLED", "true").lower()
                    == "true",
                    "trace_requests": os.getenv(
                        "TRACE_REQUESTS_ENABLED", "true"
                    ).lower()
                    == "true",
                    "collect_gpu_stats": os.getenv("COLLECT_GPU_STATS", "false").lower()
                    == "true",
                }
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not initialize enhanced GenAI tracer: {tracer_error}"
                    )
                    self.genai_tracer = None
            else:
                logger.warning(
                    "OpenLit initialization failed - continuing without observability"
                )
    ...
    def chat_with_ollama(self, messages: List[Dict[str, str]], model: str) -> str:
        """Chat with Ollama using OpenAI client pattern with OpenLit automatic instrumentation."""
        try:
            logger.info(f"Chatting with Ollama model via OpenAI client: {model}")

            # Use the OpenAI client pattern with OpenLit automatic instrumentation
            response = self.ollama_openai_client.chat_with_ollama(messages, model)

            (
                logger.info(f"Ollama response received: {response[:100]}...")
                if len(response) > 100
                else logger.info(f"Ollama response: {response}")
            )
            return response
        ...
        if self.pipelines_base_url:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        logger.info(
                            "Generating telemetry for successful Open WebUI request"
                        )
                        ...
                        logger.warning(
                            f"Failed to generate telemetry: {telemetry_error}"
                        )
    ...
    def update_all_provider_status(self) -> Dict:
        ...
        # Final guarantee - if we still don't have exactly 10, add missing providers as offline
        if len(updated_status) != 10:
            logger.warning(
                f"Provider count mismatch: expected 10, got {len(updated_status)}. Adding missing providers."
            )
            for name, info in default_providers.items():
                if name not in updated_status:
                    # Only add missing providers as red, don't override successful checks
                    updated_status[name] = {
                        "status": "🔴",
                        "response_time": "---ms",
                        "country": info["country"],
    ...
    def _automation_loop(self, model: str, interval: int):
        ...
        while not self.stop_event.is_set():
            ...
            if self.automation_send_messages:
                ...
                try:
                    ollama_reply = self.ollama_openai_client.single_prompt(
