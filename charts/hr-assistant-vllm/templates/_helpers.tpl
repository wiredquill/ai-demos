...
{{/*
Create the vLLM router endpoint URL based on release name. Matches the
Service the vendored vllm subchart's service-router.yaml renders
("{{ .Release.Name }}-router-service"), on the OpenAI-compatible port 80.
*/}}
{{- define "hr-assistant.vllmEndpoint" -}}
{{- if .Values.vllmEndpoint -}}
{{ .Values.vllmEndpoint }}
{{- else -}}
http://{{ .Release.Name }}-router-service:80
{{- end -}}
{{- end }}
...
{{/*
The primary model. Falls back to the first vLLM modelSpec entry's modelURL
(the Hugging Face repo id vLLM was told to serve).
*/}}
{{- define "hr-assistant.model" -}}
{{- if .Values.model -}}
{{ .Values.model }}
{{- else if and .Values.vllm .Values.vllm.servingEngineSpec.modelSpec -}}
{{ (first (values .Values.vllm.servingEngineSpec.modelSpec)).modelURL }}
{{- else -}}
